            ...
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"
                        ...
                        WHERE p.IsApproved = 1 AND p.IsDeleted = 0 AND p.PublishDateTime < SYSDATETIME() AND up.isDeleted = 0
            ...
            using(var conn = Connection) 
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"
                        ...
                        OUTPUT INSERTED.ID
                        VALUES (@Title, @Content, @ImageLocation, @CreateDateTime, @PublishDateTime, @IsApproved, @CategoryId, @UserProfileId, @isDeleted)";
